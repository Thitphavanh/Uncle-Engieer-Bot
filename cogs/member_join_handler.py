...
class MemberJoinHandler(commands.Cog):
    ...
    async def on_ready(self):
        print(f"Member Join Handler is online.")
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        bg = easy_pil.Editor(f"cogs/welcome_images/{randomized_image}").resize(
            (1920, 1080)
        )
        ...
        image_file = discord.File(fp=bg.image_bytes, filename=randomized_image)

        await welcome_channel.send(
            f"สวัสดีครับ, {member.name}! โปรดอ่านกฎของเราและปฏิบัติตามอย่างเคร่งครัด ขอบคุณที่เข้าร่วมเซิร์ฟเวอร์ของเรา!",
            file=image_file,
        )
